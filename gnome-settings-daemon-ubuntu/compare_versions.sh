...
UBUNTU_VER=($(wget -q -O - 'https://launchpad.net/ubuntu/raring/+source/gnome-settings-daemon' | sed -n 's/^.*current\ release\ (\(.*\)-\(.*\)).*$/\1 \2/p'))
...
echo -e "spec file version: ${F18_SPEC_VER} ${F18_UBUNTU_REL}"
