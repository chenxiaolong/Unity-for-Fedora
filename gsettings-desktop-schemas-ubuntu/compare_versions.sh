...
F17_SPEC_VER="$(rpmspec -q --qf '%{version}\n' gsettings-desktop-schemas-ubuntu-Fedora_17.spec | head -1)"
F18_SPEC_VER="$(rpmspec -q --qf '%{version}\n' gsettings-desktop-schemas-ubuntu-Fedora_18.spec | head -1)"
F17_UBUNTU_REL="$(sed -n 's/^%define[ ]*_ubuntu_rel[ ]*\(.*\)$/\1/p' gsettings-desktop-schemas-ubuntu-Fedora_17.spec)"
