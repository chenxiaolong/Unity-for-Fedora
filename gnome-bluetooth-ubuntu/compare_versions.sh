...
F17_SPEC_VER="$(rpmspec -q --qf '%{version}\n' gnome-bluetooth-ubuntu-Fedora_17.spec | head -1)"
F17_UBUNTU_REL="$(sed -n 's/^%define[ ]*_ubuntu_rel[ ]*\(.*\)$/\1/p' gnome-bluetooth-ubuntu-Fedora_17.spec)"
F18_SPEC_VER="$(rpmspec -q --qf '%{version}\n' gnome-bluetooth-ubuntu-Fedora_18.spec | head -1)"
...
UPSTREAM_VER=$(wget -q "http://ftp.gnome.org/pub/GNOME/sources/gnome-bluetooth/3.5/" -O - | sed -n 's/.*>LATEST-IS-\(.*\)<.*/\1/p')
